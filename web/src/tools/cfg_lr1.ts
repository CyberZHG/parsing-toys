...
interface LR1HashData {
    grammar: string
    input?: string
}

function getLR1HashData(): LR1HashData {
    try {
...
function setLR1HashData(data: LR1HashData): void {
...
const lr1Button = document.querySelector<HTMLElement>('#button-cfg-lr1')!
...
lr1Button.addEventListener('click', () => {
    const errorMessage = document.querySelector<HTMLTextAreaElement>('#cfg-lr1-error-message')!
    ...
    setLR1HashData({ grammar: code, input: inputString.value.trim() })
})
